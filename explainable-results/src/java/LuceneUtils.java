...
public class LuceneUtils {
    ...
    public static List<SearchResult> getSubList( List<SearchResult> results, int start, int end ) {
        List<SearchResult> sublist = new ArrayList<>();
        for ( int ix = start; ix < end; ix++ ) {
            sublist.add( new SearchResult( results.get( ix ).getDocid(), results.get( ix ).getDocno(), results.get( ix ).score ) );
        }
        return sublist;
    }

    public static List<String> dumpDocVector( IndexReader index, String field, int docid ) throws IOException {
        List<String> words = new ArrayList<>();
        TermsEnum iterator = index.getTermVector( docid, field ).iterator();
        BytesRef br;
        while ( ( br = iterator.next() ) != null ) {
            words.add( br.utf8ToString() );
        }
        return words;
    }
